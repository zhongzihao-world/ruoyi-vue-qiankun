...
export default {
  ...
  methods: {
    ...
    addChildRouteTags() {
      const { name, routerBase, path, meta } = this.childRoute;
      // console.log(this.childRoute);

      if (name && routerBase) {
        this.$store.dispatch('tagsView/addView', {
          name: routerBase + path,
    ...
    dragEnd(e) {
      ...
      const targetItem = this.visitedViews.splice(oldIndex, 1);
      this.visitedViews.splice(newIndex, 0, targetItem[0]);
    },
  },
};
</script>
